...
# 目录配置（Render使用持久化磁盘或临时存储）
BASE_DIR = Path(__file__).resolve().parent
UPLOAD_DIR = Path("/tmp/uploads")  # Render推荐使用/tmp
...
async def audit_ar_run(
    ...
    try:
        ...
        # 云部署：使用相对路径，让Node层补全完整URL
        result["download_url"] = f"/audit/ar/download/{filename}"
        result["download_full_url"] = None  # 由Node层填充
